"""
TP = 2 × SL optimizer. Take-profit is exactly twice the stop-loss.

Usage:
    python optimizer_tp2x.py --start 2026-07-22 --end 2026-07-28
    python optimizer_tp2x.py --symbols BTCUSDT,ETHUSDT --trials 100 --jobs 4
"""
...
def optimize_symbol(sym: str, base_cfg_path: str, args) -> dict:
    ...
    print(f" {len(df_raw)} candles", end="", flush=True)

    study = optuna.create_study(
        direction="maximize", study_name=f"tp2x_{cfg.symbol}",
...
def autodetect_symbols() -> list[str]:
    ...
    for f in os.listdir(bot_dir):
        if f.startswith("config_") and f.endswith(".yaml") and f != "recovery_config.yaml":
            ...
            if sym == "1000PEPEUSDT" or not any(c.isdigit() for c in sym):
                syms.append(sym)
    return sorted(set(syms))


def main():
    parser = argparse.ArgumentParser(description="TP = 2x SL optimizer")
