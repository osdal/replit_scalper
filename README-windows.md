...
### Пример строки лога
```
2026-07-21 12:34:56 [LIVE BTCUSDT] INFO Position opened | LONG | entry=67543.20 SL=67034.10 TP1=67678.40 TP2=67878.50 qty=0.0123 | indicators: ema_fast=67542.1 ema_slow=67540.5 volume=123.4 volume_ma=98.7
```
...
| `position_tracker.py:361-432` | `apply_hit` возвращает кортеж `(pnl, exit_reason)` в некоторых ветках и `float` в других. В `apply_hit_async` есть `isinstance(pnl_result, tuple)` check — хрупко. | 🔴 High |
...
| `order_manager.py:324-347` | При `recovery_qty` пересчитывается TP1 от фактического fill price, но SL остаётся из сигнала (не пересчитывается от fill). Несогласованность. | 🟡 Medium |
| `position_tracker.py:476-494` | `_sync_pnl_from_exchange` вызывается только при закрытии сделки. Unrealized PnL синкается отдельно через `sync_unrealized_pnl` (каждые 3 свечи) — возможны расхождения. | 🟢 Low |
...
## 9. Зависимости (ключевые с версиями)

Из `uv.lock` / `pyproject.toml`:
```
python-binance    >=1.0.0      # Async Binance API
pandas            >=2.0.0      # DataFrames, индикаторы
numpy             >=1.24.0     # Математика
aiohttp           >=3.8.0      # Recovery HTTP клиент
pyyaml            >=6.0        # YAML конфиги
python-dotenv     >=1.0.0      # .env загрузка
uvicorn           >=0.23.0     # API сервер (dashboard)
fastapi           >=0.100.0    # API сервер
```

Полный список — в `uv.lock` (315К строк) или `pip list` в venv.
...
4. **Что происходит при разрыве связи с Binance во время открытой позиции?** Polling продолжит попытки, но SL/TP уже стоят на бирже. Есть ли сценарий "force close" или ручное управление в таком случае?
...
7. **Как тестируется корректность PnL синхронизации с биржей?** `get_realized_pnl` использует эвристику чередования сторон — есть ли интеграционные тесты против реальной истории сделок?
