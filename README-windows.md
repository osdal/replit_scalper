...
- **`_acquire_lock`** — использует `os.kill(pid, 0)` вместо `tasklist`, кросс-платформенно, не ломается при отсутствии tasklist в PATH
- **API start handler** — проверяет `.killed` перед блокировкой рестарта. Убитые извне процессы больше не требуют ручного удаления lock-файлов
- **`findBotPid`** — проверяет наличие `main.py` в командной строке, чтобы не путать бота с оптимизатором

### Авто-очистка пылевых позиций и stale ордеров
- **Раз в 12 свечей (~1 минута)** бот проверяет все позиции на бирже:
  - Позиции с notional < $1 — закрываются маркет-ордером
  - Если позиции по символу нет и трекер пуст — отменяются все открытые ордера
- **`close_dust`** — использует `max(real_qty, step_size)` для обхода minNotional Binance

### Синхронизация сделок при старте
- **`_sync_position_on_start`** — если на бирже нет позиции, а в БД висит открытый трейд:
  - Запрашивает реальный PnL с Binance через `get_realized_pnl`
  - Закрывает трейд в БД (exit_price, pnl, exit_reason)
  - Если PnL < 0 — отправляет в recovery-цепочку
  - Предотвращает накопление «висящих» сделок после перезапусков

### Recovery
- **Recovery позиции** — TP пересчитывается под покрытие долга, qty берётся из risk-менеджмента (не `calc_recovery_quantity`)
- **`_sync_position_on_start`** — восстанавливает recovery-контекст (chain_id, is_recovery) при перезапуске
- **RecoveryTab** — кнопка удаления цепочек (корзина)

### Оптимизатор
- **`risk_pct` фиксирован на 3%** — не варьируется при оптимизации
- **HTF-параметры** — всегда генерируются (`htf_ema_fast`, `htf_ema_slow`), даже если `htf_enabled=false` в конфиге
- **SQLite-персистентность** — `--study-name` сохраняет результаты в `data/optuna.db`
- **Параллельные вычисления** — `--jobs N` для ускорения

### Дашборд
- **Backtest tab** — чекбокс HTF Filter, поля HTF EMA Fast/Slow, переносятся из Optimizer
- **Кнопка «Load to Config»** в Backtest tab — сохраняет параметры в конфиг бота
- **Кнопка «Sync Binance»** — загружает закрытые позиции со всех 24 пар (авто-определение из `config_*.yaml`)
- **Delete bot** — иконка корзины на карточке бота, удаляет из БД через `DELETE /bots/:symbol`

### Расчёт PnL
- **`get_realized_pnl`** — приоритет: Income API → userTrades fallback (для старых трейдов)
- **Income API** даёт точный PnL с учётом комиссий от Binance
