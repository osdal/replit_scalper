...
### Автоматическая симуляция отклонённых сигналов в реальном времени
Начиная с версии `main.py`, бот **сам заполняет** `exit_reason`, `exit_price`, `pnl` и `qty` для отклонённых сделок без ручного запуска скрипта:

1. **При отклонении сигнала** (`risk:max_positions`, `risk:debt_limit` и т.д.):
   - Бот рассчитывает hypothetical `qty` по той же логике, что и для реальных сделок (`margin_pct` → `fixed_notional_usd` → `fixed_qty` → `fixed_risk_usd` → `risk_pct`).
   - В `trades` создаётся запись `status='rejected'` с посчитанным `qty`.

2. **В реальном времени** (`_simulate_rejected_outcome`):
   - Каждые 10 секунд (на каждой свече) проверяется, достигла ли текущая цена `SL` или `TP1`.
   - При касании уровня записывается `exit_reason`, `exit_price`, `pnl` (реальный PnL в USDT, расчитанный как `qty × (exit_price − entry_price)` для LONG, обратно для SHORT).

3. **Фоновая историческая симуляция** (`_simulate_rejected_background`):
   - Раз в 60 секунд проверяются все отклонённые сделки без `exit_reason`.
   - Скачиваются 5m-свечи с момента `entry_time` и проверяется, какой уровень был достигнут первым.
   - Результат записывается в БД с реальным PnL.

4. **При старте бота** (`_load_pending_rejected`):
   - Из БД загружаются все `status='rejected'` без `exit_reason` в очередь симуляции.
   - Это гарантирует, что после перезапуска бота симуляция продолжится автоматически.

В дашборде отклонённые сделки отображаются с:
- `⛔ REJECTED (причина)` — статус отклонённой сделки
- `sim: TP1` / `sim: SL` — результат симуляции
- `qty` — расчитанный объём позиции
...
2. `/start` или кнопка **🚀 Запустить бота** → главное меню: **Подключить бота**, **Мои ключи**, **Поддержка**.
3. **Подключить бота** → бот запрашивает API-key → API-secret → валидация через Binance → сохранение.
4. Если ключи уже подключены и активны → кнопка меняется на **Заменить ключи**.
5. При замене ключей: новая валидация → обновление записи.
6. Невалидные ключи **не сохраняются**.
7. **Мои ключи** → статус подключения и маскированный API-key.
8. **Поддержка** → переадресация в чат с администратором с предзаполненным текстом.

### Модель данных
- Один пользователь = **один набор Binance API-ключей** (без привязки к символу/режиму).
- Таблица `user_credentials` с `UNIQUE(user_id)`.
- Поддерживается активация/деактивация ключей.
...
## 17. Договорённости и исправления (26 августа 2026) — опорная ревизия

> ⚠️ **Эта секция — канон текущего поведения.** Если что-то меняется в будущем — начинать от этих правил.

### Ключевая договорённость: отклонённая сделка = обычная, но помеченная
Все сделки обрабатываются **идентично**, включая отклонённые:

- Отклонённая сделка (из-за лимита `max_positions`, паузы `loss_streak` и пр.) **открывается как настоящая позиция**:
  трекер (`position_tracker.py`), SL/TP, полное закрытие, отображается в боте/карточке.
- Отличие ровно одно — в записи БД ставится `status='rejected'` + `reject_reason` (напр. `max_positions`), из-за чего:
  - сделка **исключается** из статистики винрейта/PnL (`status != 'rejected'` фильтр);
  - сделка **не занимает слот** в лимите `max_positions` (счётчик считает только `status != 'rejected'`).
- В дашборде отображается бейдж `⛔ REJECTED (причина)`.

### Лимит позиций (`max_positions`)
- Сервер `POST /api/trading/check` считает открытые позиции = `is_open=1 AND status != 'rejected'`.
- `countOpenPositions()` учитывает **только принятые** (rejected не входят).
- При `positions_open >= max_positions` новый сигнал **отклоняется** (открывается как rejected-позиция).
- **При закрытии реальной (принятой) сделки** `is_open` → 0, счётчик уменьшается на 1 → освобождается слот → следующий сигнал открывается как **обычная** (не rejected).
- Не более `max_positions=2` принятых позиций одновременно.

### Пауза после серии убытков (`loss_streak`)
- Реальная убыточная сделка инкрементирует общий `loss_streak`.
- При `loss_streak >= loss_streak_trigger` (2) включается пауза (`paused_remaining=1`) — новые входы отклоняются.
- **Выход из серии** — только прибыльная сделка:
  - реальная прибыльная (`simulated=false`, `pnl>=0`) снимает паузу;
  - rejected-позиция, закрывшаяся в плюс (`simulated=true`, `pnl>=0`), также снимает паузу.
- Отклонённые в паузу открываются как rejected-позиции, не считаются в статистике.

### `POST /api/trading/close-and-reset` (кнопка «Close All & Reset» в дашборде)
Одна кнопка (в шапке дашборда) выполняет:
1. Закрывает ВСЕ открытые сделки в `trades` (принятые и rejected) → `is_open=0, status=closed, exit_reason=manual_reset`.
2. Удаляет старые `manual_reset` записи (остатки прошлых сбросов).
3. Удаляет отклонённые (`rejected`) записи без итогового `pnl` (мусор — «висящие» REJECTED без закрытия).
4. Сбрасывает `position`/`is_running`/`current_price` у всех ботов в таблице `bots` (чистит карточки).
5. Обнуляет `loss_streak` и `paused_remaining` в `trading_control`.
6. Останавливает всех ботов (fire-and-forget) и удаляет `state_*`/`bot.lock.*` файлы.

### Fail-closed при недоступности сервера
`bot/recovery_client.py::can_open()` — при ЛЮБОЙ ошибке/недоступности `/trading/check`
(сеть, статус != 200, исключение) возвращает `{"allowed": false, "reason": "check_error"}`
(fail-closed) — бот **не открывает** позицию сверх лимита при сбое API (ранее был fail-open,
что позволяло открывать лишние позиции в момент рестарта сервера).

### Живой Unrealized PnL в дашборде
`bot/main.py::tick_sl_tp_check()` — при открытой позиции каждые ~5 сек берёт live-тикер
Binance и шлёт `report_heartbeat(current_price)`, чтобы `current_price` в БД/карточке обновлялся
между свечами и unrealized PnL пересчитывался в реальном времени.
`bot/db_reporter.py::report_heartbeat()` защищает от записи цены `<= 0` (Binance при лимите возвращает 0 →
ложный PnL в дашборде).

### Блокировка серии убытков как причина спама (важно)
Ранее пауза `loss_streak` блокировала ВСЕ входы даже при свободном слоте лимита →
дашборд засорялся записями `⛔ REJECTED (loss_streak)`. Поведение уточнено:
- пауза снимается прибыльной сделкой (см. выше);
- rejected-позиции открываются как позиции (не плодят отдельные записи-сироты).

### Прочие исправления этой ревизии
- `bot/strategy.py::detect_engulfing_pattern` — возвращал строку `"engulfing_long"` вместо
  кортежа `("engulfing_long","LONG")`, из-за чего engulfing-сигналы падали
  (`ValueError: too many values to unpack`) и позиции не открывались. Исправлено.
- `bot/main.py` — устранён `NameError: open_reject_reason` (остаточный вызов в `tracker.open_async`).
- `lib/db/schema/trading.ts` + `init-db.ts` — колонка `trading_control.active_open` (миграция).
- `README` — канон поведения этой ревизии.

### Статус этой ревизии
> По состоянию на 26.08.2026: **всё работает** — лимиты, пауза, rejected-сделки, кнопка сброса,
> live-PnL, запуск 35 ботов из дашборда. С этой ревизии опираться в дальнейших доработках.
