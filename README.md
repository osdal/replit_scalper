...
**Режим пресета (`mode`)** — опциональный ключ в конфиге конкретного пресета:

```python
"macd_hist_trend_long": {"tp": 1.0, "sl": 0.5, "max_per_preset": 0, "mode": "live"},  # торговать этим пресетом live
"stoch_bounce_long":     {"tp": 0.7, "sl": 0.35, "max_per_preset": 0},                  # наследует режим бота (cfg.mode)
```

- `"mode": "paper"` / `"mode": "live"` — переопределяет режим торговли для этого пресета.
- **Приоритет выше, чем у конфига**: если пресет `live`, а бот в `paper` — сделка по этому пресету открывается live. И наоборот.
- Если ключ отсутствует — пресет наследует режим бота из `config_<symbol>.yaml` (`mode: paper|live`).
- Возможно смешивание: например бот в `paper`, но часть пресетов помечена как `live` — такие сделки открываются реальными ордерами, остальные симулируются (бумажные).
- **Внимание**: для live-пресета требуются валидные `BINANCE_API_KEY`/`BINANCE_API_SECRET`; иначе ордер упадёт с ошибкой авторизации (безопасно — реальные сделки не будут открыты).
...
# AUTO_RESTART_BOTS=true (по умолчанию): если при старте api-server бот был
# is_running=1, но его процесс не выжил (рестарт/падение grid-stack), бот
# поднимается заново (до 3 попыток на символ). "false" отключает авто-рестарт.
...
**Защита от серии убытков (глобальная):**
- Счётчик убытков ведётся **по всему портфелю** (все монеты суммируются).
- Реальный убыток (SL) → `+1`; реальная прибыль (TP1/TP2) → сброс в `0` и снятие паузы.
- При `loss_streak_trigger` убытков подряд включается пауза на `loss_pause_signals` сигналов — новые входы отклоняются с причиной `risk:loss_streak`.
- Симуляция отклонённых сигналов (TP/SL) пишется только в аналитику и **не влияет** на паузу/счётчик.
- Автосброс: через `pause_timeout_minutes` без сделок пауза и счётчик обнуляются.
...
## 17. Grid-бот (dashboard-v2 + api-server)

### 17.1 Что это и чем отличается от scalper-ботов

Grid-бот — это связка **dashboard-v2 (порт 5175)** + **api-server (порт 5000)**, которая выставляет **реальные ордера на Binance USDⓈ-M Testnet** через роуты `/api/grid-orders/*`. Это **не Python-процесс**: состояние сетки (уровни, лоты, avg, TP, PnL) живёт в браузере, а api-server только подписывает и отправляет запросы к Binance.

> Scalper-боты (`bot/main.py` + `config_*.yaml`, разделы 1–13) — отдельная система. Grid-бот их не запускает, не останавливает и не использует их конфиги.

### 17.2 Двусторонняя сетка

- Уровни **выше `mid`** торгуются только как **SELL**, уровни **ниже `mid`** — только как **BUY**.
- Уровень ровно на `mid` **не торгуется** (пропускается с reason `mid level`).
- Сторона уровня выводится из `midPrice` на сервере (`levels[] + midPrice`); клиент может прислать готовый `orders[]`.

### 17.3 Выходы из позиции (динамический TP)

Для каждой стороны TP пересчитывается каждый тик и жёстко ограничен `mid`, чтобы не уйти на противоположную сторону диапазона:

```
longTp  = min(avgLong  * (1 + tpPct / 100), mid)
shortTp = max(avgShort * (1 - tpPct / 100), mid)
```

- **SL** — на границах диапазона (`lo`/`hi`), выставляется как условный ордер на бирже.
- Закрытие по TP/SL — **реальный MARKET-ордер** с `reduceOnly`.

### 17.4 Защита на стороне биржи (Algo API)

Условные ордера (SL/TP) ставятся **только через Algo API**:

- `POST /fapi/v1/algoOrder` с `algoType=CONDITIONAL`, `triggerPrice`, `closePosition=true`, `workingType=MARK_PRICE`, `clientAlgoId`.
- Обычный `POST /fapi/v1/order` с `STOP_MARKET` / `TAKE_PROFIT_MARKET` для grid-защиты возвращает **`-4120`**, поэтому не используется.
- Защита ставится **только при наличии позиции**; иначе Binance отвечает `-4509`.
- Amend для algo-ордеров отсутствует — смена TP выполняется как **cancel + place**, `algoId` при этом меняется.

### 17.5 Синхронизация TP

- **Авто**: при торговых событиях — первый/новый филл, закрытие стороны.
- **Вручную**: если изменился только `tpPct`, TP на бирже не переставляется автоматически — нужно нажать **Push TP to Binance**.
- Индикатор на дашборде показывает состояние на бирже: **TP on exchange** / **TP not applied**.

### 17.6 Изоляция от scalper-ботов

- `auto_mode` пропускает символы, где уже запущен бот или открыта позиция.
- Ручное создание сетки по «занятому» символу возвращает **409** (`symbol is not in GRID_SYMBOLS`, пауза риска и т.п.).
- Опциональный allowlist `GRID_SYMBOLS`; при активной глобальной паузе торговли (`paused_remaining`) новые grid-входы отклоняются.
- Grid-бот **не вызывает `cancelAll`** — защитные ордера снимаются только по своим `algoId` (`/cancel-stops`).

### 17.7 Rate-limit оптимизации

- **WS mark-price кэш** для цен (`!markPrice@arr@1s`), fallback на REST.
- **ADX-кэш 300 с** + staggered старт по символам (`SYMBOL_STAGGER_MS`), чтобы не флудить klines.
- **Батчированный сбор филлов** (`/fills`) + опрос раз в 30 с.
- **Retry/backoff** на `-1003` / `-429` с ограничением ожидания.

### 17.8 Полный сброс аккаунта

Красный блок **Danger zone** → модалка с **отменяемым 5-секундным отсчётом** → `POST /api/grid-orders/reset` (полная очистка аккаунта на тестнете). Состояние отсчёта — `GET /api/grid-orders/reset-status`; дашборд автоматически очищает локальные сетки, если серверный `resetAt` новее локально запомненного.
...
|------------|-----------|
| `BINANCE_TESTNET=true` | Grid-роуты работают только против Testnet |
| `GRID_ORDERS_ENABLED=true` | Явно разрешает серверные grid-роуты |
| `NOTIFY_TOKEN` | Токен для мутирующих роутов (fail-closed; без него — `503`) |
| `GRID_SYMBOLS` (опц.) | Allowlist символов через запятую; пусто = без ограничений |

Дашборд отправляет токен из `VITE_NOTIFY_TOKEN` в заголовке `x-notify-token`.

### 17.10 Основные API-роуты (`/api/grid-orders`)

| Метод | Роут | Назначение |
|-------|------|-----------|
| POST | `/create` | Создать двустороннюю сетку |
| POST | `/stop` | Снять защитный STOP |
| POST | `/tp` | Переставить TP (cancel + place) |
| POST | `/cancel-stops` | Снять все grid-стопы по своим algoId |
| POST | `/cancel` | Отменить ордера сетки |
| POST | `/fills` | Батч-сбор исполнений |
| POST | `/close` | Реальное закрытие позиции (MARKET, reduceOnly) |
| GET | `/open-algo` | Открытые algo-ордера |
| POST | `/algo-status` | Статус algo-ордеров |
| POST | `/reset` | Полный сброс тестнет-аккаунта |
...
- `closePosition=true` закрывает **всю нетто-позицию**, а не только лоты конкретной сетки.
