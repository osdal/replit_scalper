/**
 * Инициализация БД: создаёт таблицы и добавляет ботов из yaml конфигов.
 * Запуск: npx tsx src/init-db.ts
 */
...
db.run(sql`CREATE TABLE IF NOT EXISTS bots (
  ...
  tp1_close_pct REAL NOT NULL,
  tp2_pct REAL NOT NULL,
  ema_fast INTEGER NOT NULL,
  ema_slow INTEGER NOT NULL,
  volume_ma_period INTEGER NOT NULL,
  volume_multiplier REAL NOT NULL,
  htf_enabled INTEGER NOT NULL DEFAULT 0,
  htf_timeframe TEXT,
  htf_ema_fast INTEGER,
  htf_ema_slow INTEGER,
  auto_mode INTEGER NOT NULL DEFAULT 1,
  paper_balance REAL NOT NULL DEFAULT 1000,
...
// Читаем все config_*.yaml из папки бота
const configs = fs.readdirSync(BOT_DIR).filter(f => f.match(/^config_\w+\.yaml$/) && f !== "config.yaml");
...
for (const file of configs) {
  ...
  const existing = db.select().from(botsTable).where(sql`symbol = ${symbol}`).get();
  if (existing) {
    console.log(`  ${symbol} already exists — skipping`);
    continue;
  }

  db.insert(botsTable).values({
    symbol,
